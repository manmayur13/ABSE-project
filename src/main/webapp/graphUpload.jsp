...
if (request.getParameter("dedup") != null) {
    ...
    <div id="featured">
			...
			PreparedStatement ps = con.prepareStatement("select * from graph1 where fileid=? and userid=?");
			ps.setString(1, request.getParameter("file_id"));
			ps.setString(2, session.getAttribute("userid").toString());
			ResultSet rs = ps.executeQuery();
			if(rs.next())
			{
				enctime = rs.getString("enctime");
		...
		window.onload = function () {
			var chart = new CanvasJS.Chart("chartContainer", {
				...
				data: [{
					type: "column",
					dataPoints: [
						{ y: <%=enctime%>, label: "Encryption Time" },
						{ y: <%=uploadtime%>, label: "Upload Time" },
