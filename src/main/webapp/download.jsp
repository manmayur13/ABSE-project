  ...
  <body>
   ...
   <div id="featured">
	...
	while(rs.next())
	{
		...
		String lsdate=rs.getString("last_date");
		...
		<td><%=email %></td>
		<td><%=filename %></td>
		<td><a href="Download?filename=<%=filename%>&file_id=<%=file_id%>&lsdate=<%=lsdate%>">Download</a></td>
		<%
	}
	...
	while(rs1.next())
	{
		...
		String file_id=rs1.getString("id");
		String lsdate=rs1.getString("last_date");
		%>
		<tr>
		<td><%=srno++ %></td>
		<td><%=fname %></td>
